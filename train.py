...
def parse():
    ...
    parser.add_argument('--log', type=str, default='inception_mixed_7c.log')
    parser.add_argument('--net', type=str, default='inception_mixed_7c')
...
def train(opt):
    ...
    with tqdm.tqdm(total=opt.epoch, miniters=1, mininterval=0) as progress:
        for epoch in range(1, opt.n_epochs+1):
            ...
            for i, batch in enumerate(train_loader, 1):
                ...
                batch_loss = loss(outputs, correct_label)/1.5 + loss(y_pred_crop, correct_label)/2.\
    ...
    predict(opt, 'train_wsdan_{}.pth'.format(opt.net))


def predict(opt, model_name):
    testLoader = DataLoader(testData('./'), batch_size=1, num_workers=2)
    model = torch.load(model_name).cuda()
    model.eval()
    with open('output_{}.csv'.format(model_name), 'w', newline='') as csvfile:
...
if __name__ == '__main__':
    opt = parse()
    train(opt)
    # predict(opt, 'train_wsdan_{}.pth'.format('resnet34'))