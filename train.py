...
def parse():
    ...
    parser.add_argument('--data', type=str, default='3')
    ...
    parser.add_argument('--log', type=str, default='inception_mixed_6e.log')
...
def train(opt):
    ...
    data = Data('./split_data/train{}.csv'.format(opt.data))
    valdata = Data('./split_data/val{}.csv'.format(opt.data), phase='val')
    val_min = float('inf')
    train_loss = float('inf')

    train_loader = torch.utils.data.DataLoader(data, batch_size=opt.batch, num_workers=opt.n_cpu)
    val_loader = torch.utils.data.DataLoader(valdata, batch_size=1, num_workers=opt.n_cpu)
    with tqdm.tqdm(total=opt.epoch, miniters=1, mininterval=0) as progress:
        for epoch in range(1, opt.n_epochs+1):
            ...
            for i, batch in enumerate(train_loader, 1):
                ...
                raw_rate = raw_rate + epoch_raw_acc[0]*opt.batch
                crop_rate = crop_rate + epoch_crop_acc[0]*opt.batch
                drop_rate = drop_rate + epoch_drop_acc[0]*opt.batch

                if i % 1 == 0:
                    des = "Epoch: {epoch}, Iter {iter:.1f}%, Total Loss: {loss:.4f} ".format(
                        epoch=epoch,
                    ...
                    des_loss = "Raw Acc: {r1:.2f}, Crop Acc: {c1:.2f}, Drop Acc: {d1:.2f}, lr: {lr:.5f}".format(
                                                                lr=scheduler.get_lr()[0], r1=raw_rate/len(data),
                                                                c1=crop_rate/len(train_loader), d1=drop_rate/len(data))
                    progress.set_description(des+des_loss)
            des = "Epoch: {epoch}, Total Loss: {loss:.4f} ".format(
                epoch=epoch,
                loss=epoch_loss,
            )
            des_loss = "Raw Acc: {r1:.2f}, Crop Acc: {c1:.2f}, Drop Acc: {d1:.2f}, lr: {lr:.5f}".format(
                lr=scheduler.get_lr()[0], r1=epoch_raw_acc[0],
                c1=epoch_crop_acc[0], d1=epoch_drop_acc[0])
            print(des+des_loss, end='')
            logging.info('{}'.format(des+des_loss))

            if epoch_loss < train_loss:
                train_loss = epoch_loss
                torch.save(model, './split_data/train_wsdan_{}.pth'.format(opt.net))
                print('\t Train Model Saved', end='')
            ...
            test_loss = test_loss/len(valdata)
            des = "\nTest Loss: {loss:.5f}, acc: {a1:.2f}".format(
                loss=test_loss,
                a1=epoch_acc[0])
            print(des)
            ...
            if test_loss < val_min:
                val_min = test_loss
                torch.save(model, './split_data/val_wsdan_{}.pth'.format(opt.net))
...
def predict(opt, model):
    testLoader = DataLoader(testData('./test.csv'), batch_size=1, num_workers=2)
    model = torch.load(model).cuda()
    model.eval()
    with open('./split_data/output{}.csv'.format(opt.data), 'w', newline='') as csvfile:
...
if __name__ == '__main__':
    opt = parse()
    train(opt)
    predict(opt, './bagging_model/val_wsdan_{}.pth'.format(opt.net))